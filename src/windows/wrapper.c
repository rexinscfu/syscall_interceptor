...
extern int _hook_syscall(const char* syscall_name, syscall_handler_t handler);
extern int _unhook_syscall(const char* syscall_name);
...
int hook_syscall(const char* syscall_name, syscall_handler_t handler) {
    return _hook_syscall(syscall_name, handler);
}

int unhook_syscall(const char* syscall_name) {
    return _unhook_syscall(syscall_name);
}
